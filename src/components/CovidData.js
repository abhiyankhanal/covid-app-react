...
super(props)
  ...
  handleSubmit = () => {
    console.log(this.state.searchValue)
    this.getCovidData(this.state.searchValue);
    ...
    return (
      <>
        ...
        <form className="myForm">
          <label>
            Pick the country:
            <select value={this.state.searchValue} onChange={(e) => this.handleChange(e.target.value)}>

              {country_list.map((el) => <option value={el} key={el}>{el}</option>)}

            </select>
          </label>
          <button className='searchButton' type = 'button' onClick={this.handleSubmit}>Search</button>
        </form>
